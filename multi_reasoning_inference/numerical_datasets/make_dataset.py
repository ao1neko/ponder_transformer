...
def main(args):
    ...
    number_set_list = [json_number_set["train_number_set"],json_number_set["valid_number_set"],json_number_set["test_number_set"]]
    ...
    for number_set,data_size,method in zip(number_set_list,data_size_list,method_list):
        ...
        with open(Path(output_file), 'w') as json_file:
            while data_size > maked_data_size :
                data_instance = DataInstance(useable_char_set= set(string.ascii_uppercase),useable_int_set=number_set)
                data_instance.make_instance(inference_step=inference_step,equation_num=equation_num)
