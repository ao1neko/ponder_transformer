...
class BartLearnedPositionalEmbedding(nn.Embedding):
    ...
    def forward(self, input_ids_shape: torch.Size, past_key_values_length: int = 0):
        ...
        if self.training:
            ...
            positions = positions + random_offset
...
class BartModel(BartPretrainedModel):
    ...
    def forward(
        ...
        #embedding層の出力(0) + encoder層の出力(base 1~6 or large 1~12)
        for hidden_state in encoder_outputs[1][1:]:
            ...
            outputs_list.append(decoder_outputs.last_hidden_state)
        if not return_dict:
...
class ReasoningBartForConditionalGeneration(BartPretrainedModel):
    ...
    def forward(
        ...
        for output in outputs_list:
            lm_logit = self.lm_head(output) + self.final_logits_bias
            lm_logits = lm_logit if lm_logits is None else torch.cat((lm_logits, lm_logit), dim=1)
            if labels is not None:
                loss_fct = CrossEntropyLoss()
                masked_lm_loss += loss_fct(lm_logit.view(-1, self.config.vocab_size), labels.view(-1))
