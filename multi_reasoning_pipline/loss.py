...
class ReconstructionLoss(nn.Module):
    ...
    def forward(self, p, y_pred, y_true, pad_id=0):
        num_layers, batch_size, seq_len, _ = y_pred.shape
        y_pred = rearrange(y_pred, 'n b s d-> n d b s')
        y_pred_sequence= y_pred[:,:,:,:-1].clone()
        y_true_sequence = y_true[:,1:].clone()
        y_true_sequence = y_true_sequence.expand(num_layers,batch_size,seq_len-1)
...
class RegularizationLoss(nn.Module):
    def __init__(self, lambda_p, device=torch.device('cpu'), num_layers=20):
        super().__init__()

        p_g = torch.zeros((num_layers,)).to(device)
        not_halted = 1.0
    ...
    def forward(self, p):
        ...
        p = p.transpose(0, 1)  # (batch_size, num_layers)
